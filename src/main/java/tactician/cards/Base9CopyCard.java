...
public abstract class Base9CopyCard extends BaseCard {
	private static final UIStrings uiStrings = CardCrawlGame.languagePack.getUIString(TacticianMod.makeID("Label9Copy"));

	public Base9CopyCard(String ID, CardStats info) {
		this(ID, info, getCardTextureString(removePrefix(ID), info.cardType));
	}

	public Base9CopyCard(String ID, CardStats info, String cardImage) {
		this(ID, info.baseCost, info.cardType, info.cardTarget, info.cardRarity, info.cardColor, cardImage);
	}

	public Base9CopyCard(String ID, int baseCost, CardType cardType, CardTarget cardTarget, CardRarity cardRarity, CardColor cardColor, String cardImage) {
	...
	public static class RarityTipModifier extends AbstractCardModifier {
		public List<TooltipInfo> additionalTooltips(AbstractCard card) {
			...
			tips.add(new TooltipInfo(Base9CopyCard.uiStrings.TEXT[0], Base9CopyCard.uiStrings.TEXT[1]));
			return tips;
		}
