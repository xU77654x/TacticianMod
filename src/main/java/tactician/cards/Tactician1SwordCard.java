...
public abstract class Tactician1SwordCard extends TacticianCard {
	private static final UIStrings uiStrings = CardCrawlGame.languagePack.getUIString(TacticianMod.makeID("Label1Sword"));

	public Tactician1SwordCard(String ID, CardStats info) {
		this(ID, info, getCardTextureString(removePrefix(ID), info.cardType));
	}

	public Tactician1SwordCard(String ID, CardStats info, String cardImage) {
		this(ID, info.baseCost, info.cardType, info.cardTarget, info.cardRarity, info.cardColor, cardImage);
	}

	public Tactician1SwordCard(String ID, int baseCost, CardType cardType, CardTarget cardTarget, CardRarity cardRarity, CardColor cardColor, String cardImage) {
	...
	public void trickyInitializeTitle() { initializeTitle(); } // This adds the "| Sword" to the middle of the card.

	public static class RarityTipModifier extends AbstractCardModifier {
		public List<TooltipInfo> additionalTooltips(AbstractCard card) {
			ArrayList<TooltipInfo> tips = new ArrayList<>();
			tips.add(new TooltipInfo(Tactician1SwordCard.uiStrings.TEXT[0], Tactician1SwordCard.uiStrings.TEXT[1])); // This adds a tooltip explaining the Sword's strong and weak weapons.
			return tips;
		}
