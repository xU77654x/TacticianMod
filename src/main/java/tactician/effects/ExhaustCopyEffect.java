...
public class ExhaustCopyEffect extends AbstractGameEffect {
    ...
    public ExhaustCopyEffect(AbstractCard srcCard, float x, float y) {
        ...
        if (this.card.type != AbstractCard.CardType.CURSE && this.card.type != AbstractCard.CardType.STATUS && AbstractDungeon.player
    ...
    public ExhaustCopyEffect(AbstractCard card) {
        ...
        if (card.type != AbstractCard.CardType.CURSE && card.type != AbstractCard.CardType.STATUS && AbstractDungeon.player
                .hasPower("MasterRealityPower"))
            card.upgrade();
        AbstractDungeon.player.exhaustPile.addToTop(card); // Changed to Exhaust pile.
    }
    ...
    public void update() {
        ...
        this.duration -= Gdx.graphics.getDeltaTime();
        if (!this.card.fadingOut && this.duration < 0.5F &&
