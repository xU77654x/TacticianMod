...
public class Wiz {
    ...
    // TODO: Cards generated by Gamble show as being 0 cost in the Compendium for a bit.
    public static AbstractCard randomCombatArt(boolean costZero) {
        ArrayList<AbstractCard> list = new ArrayList<>();
        for (AbstractCard c : srcCommonCardPool.group) { if (c.hasTag(CustomTags.COMBAT_ART)) { list.add(c); }}
        for (AbstractCard d : srcUncommonCardPool.group) { if (d.hasTag(CustomTags.COMBAT_ART)) { list.add(d); }}
        for (AbstractCard e : srcRareCardPool.group) { if (e.hasTag(CustomTags.COMBAT_ART)) { list.add(e); }}
        AbstractCard result = list.get(cardRandomRng.random(list.size() - 1));
        if (costZero) { result.setCostForTurn(0); }
        return result;
    }
