	...
	public static AbstractPower enemyWeaponCalc (AbstractMonster m, AbstractMonster.Intent intent) {
		...
		switch (m.id) { // You can add cases for non-vanilla enemies using quotes without the ID.
			...
			case Hexaghost.ID:
				...
				if (intent == ATTACK) { pow = new StrengthPower(m, 999); } // Hexaghost has two different non-debuff attacks.
				if (intent == ATTACK_DEBUFF) { pow = new StrengthPower(m, 999); } break; // Hexaghost has two different debuff attacks.

			// Standard Act 2
			case Byrd.ID:
				if (intent == ATTACK) { pow = new StrengthPower(m, 999); } // Byrd has three different attacks.
			...
			case Chosen.ID:
				if (intent == ATTACK) { pow = new StrengthPower(m, 999); } // Chosen has two different attacks.
				if (intent == ATTACK_DEBUFF) { pow = new Weapon5WindPower(m); }
				if (intent == DEBUFF) { pow = new Weapon4BowPower(m); }
				if (intent == STRONG_DEBUFF) { pow = new Weapon8DarkPower(m); } break;
			case Centurion.ID:
				if (intent == ATTACK) { pow = new Weapon3AxePower(m); } // Centurion has two different attacks; both will use Sword.
				if (intent == DEFEND) { pow = new Weapon8DarkPower(m); } break;
			case Healer.ID:
				if (intent == BUFF) { pow = new StrengthPower(m, 999); } // Mystic's heal and Strength gain are both Buff intentions.
			...
			case Darkling.ID:
				...
				if (intent == BUFF || intent == UNKNOWN) { pow = new Weapon0NeutralPower(m); }
				if (intent == ATTACK) { pow = new StrengthPower(m, 999); } break; // 2 attacks
			...
			case AwakenedOne.ID:
				...
				if (intent == ATTACK_DEBUFF) { pow = new Weapon4BowPower(m); }
				if (intent == ATTACK) { pow = new StrengthPower(m, 999); } break; // 4 attacks
			case TimeEater.ID:
